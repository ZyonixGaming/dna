(function () {
    ...
    function create(opts) {
        ...
        function render() {
            ...
            categories.forEach(function (cat) {
                ...
                renameBtn.addEventListener('click', async function (e) {
                    e.stopPropagation();
                    var newName = await doPrompt('New category name:', cat.name);
                ...
                deleteBtn.addEventListener('click', async function (e) {
                    e.stopPropagation();
                    if (await doConfirm('Delete category "' + cat.name + '" and all its DNA?')) {
                        reloadCategoriesFromStorage();
                ...
                if (cat.id === selectedCategoryId) {
                    ...
                    cat.entries.forEach(function (entry) {
                        ...
                        entryRename.addEventListener('click', async function (e) {
                            e.stopPropagation();
                            var newName = await doPrompt('New name:', entry.name);
                            if (newName && newName.trim()) {
                                reloadCategoriesFromStorage();
                                var targetCat = categories.find(function (c) { return c.id === cat.id; });
                                if (targetCat) {
                        ...
                        entryDelete.addEventListener('click', async function (e) {
                            e.stopPropagation();
                            if (await doConfirm('Delete DNA "' + entry.name + '"?')) {
                                reloadCategoriesFromStorage();
                                var targetCat = categories.find(function (c) { return c.id === cat.id; });
                                if (targetCat) {
        ...
        async function addCategory() {
            var name = await doPrompt('Category name:');
        ...
        async function saveCurrentDna() {
            ...
            var cat = categories.find(function (c) { return c.id === selectedCategoryId; });
            if (!cat) return;
            var name = await doPrompt('Name for this DNA snapshot:', 'DNA ' + new Date().toLocaleTimeString());
            if (!name || !name.trim()) return;
            reloadCategoriesFromStorage();
            cat = categories.find(function (c) { return c.id === selectedCategoryId; });
            if (!cat) { toast('That category was deleted in another window', 'error'); return; }
        ...
        function destroy() {
            destroyed = true;
            if (syncChannel) {
                try { syncChannel.onmessage = null; syncChannel.close(); } catch (e) {   }
            }
