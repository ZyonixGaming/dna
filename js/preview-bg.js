/* ======================================================================
   preview-bg.js - background swatches for a horse preview. One global:
   window.PreviewBg.

     PreviewBg.attach({
       bar,          // Element or selector: the swatch row is rendered into it
       target,       // Element or selector: whose CSS background is set
       storageKey,   // optional localStorage key (per-tool UI state, never synced)
       colors,       // optional [{ value, title }]; defaults to PreviewBg.COLORS
       defaultValue  // optional; first color if omitted
     })  -> { set(value), get() }

   The swatches stay on ONE row: the bar has a fixed height and wraps into
   hidden overflow, so on a narrow panel the last swatches drop out whole
   instead of squashing. Put the most useful colors first.
   ====================================================================== */
(function (root) {
    'use strict';
    ...
    // Same set as the visual editor's preview toolbar, plus the pages' own default.
    const COLORS = [
